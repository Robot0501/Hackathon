...
import { ShieldCheck, Building2, Check, X, Trash2, Calendar, Sparkles } from 'lucide-react-native';
import { useApp } from '../context/AppContext';
import { theme } from '../theme';
import { RichfieldEvent, UserProfile } from '../types';
...
export default function AdminScreen() {
  ...
  const [aiReviewTarget, setAiReviewTarget] = useState<UserProfile | null>(null);
  const [aiReview, setAiReview] = useState<any>(null);
  const [aiLoading, setAiLoading] = useState(false);

  const realUsers = users.filter((u) => !u.isShowcase);
  const pendingBusinesses = realUsers.filter((u) => u.role === 'business' && u.businessDetails?.approvalStatus === 'pending');
  const pendingOpps = opportunities.filter((o) => o.status === 'pending_approval');
  const students = realUsers.filter((u) => u.role === 'student');
  const alumni = realUsers.filter((u) => u.role === 'alumni');
  const companies = realUsers.filter((u) => u.role === 'business');
  const administrators = realUsers.filter((u) => u.role === 'admin');

  const localBusinessReview = (biz: UserProfile) => {
    const details = biz.businessDetails;
    const checks = [
      { label: 'Recruiter email verified by Supabase Auth', ok: biz.verificationStatus !== 'unverified' },
      { label: 'Organisation name supplied', ok: !!details?.organizationName?.trim() },
      { label: 'Registration/CIPC reference supplied', ok: !!details?.registrationNumber?.trim() && !details.registrationNumber.toLowerCase().includes('pending') },
      { label: 'Company website supplied', ok: !!details?.website?.trim() },
      { label: 'Company description supplied', ok: !!details?.companyDescription?.trim() },
      { label: 'Recruiter contact email supplied', ok: !!details?.contactEmail?.trim() },
    ];
    ...
    return {
      ...
      checks,
      summary: score >= 80
        ? 'The submitted information is internally consistent and contains most of the expected evidence. The administrator should still confirm the organisation externally before approval.'
        : 'Some verification evidence is missing or incomplete. Request supporting information or manually verify the organisation before approval.',
      disclaimer: 'AI-assisted review only. Final approval remains with the authorised Richfield administrator.',
    };
  };

  const runBusinessAiReview = async (biz: UserProfile) => {
    setAiReviewTarget(biz);
    setAiReview(null);
    setAiLoading(true);
    try {
      ...
      if (apiUrl) {
        ...
        try {
          const response = await fetch(`${apiUrl}/api/gemini/admin-verification`, {
            method: 'POST',
      ...
      setAiReview(localBusinessReview(biz));
    } catch {
      setAiReview(localBusinessReview(biz));
    } finally {
      setAiLoading(false);
  ...
  const renderUserGroup = (title: string, group: UserProfile[]) => (
    <View style={styles.userGroup}>
      <View style={styles.userGroupHeader}><Text style={styles.userGroupTitle}>{title}</Text><View style={styles.groupCount}><Text style={styles.groupCountText}>{group.length}</Text></View></View>
      {group.length === 0 ? <Text style={styles.groupEmpty}>No registered {title.toLowerCase()} yet.</Text> : group.map((u) => (
        <View key={u.id} style={styles.userRow}>
          <UserAvatar uri={u.avatar} name={u.name || u.email} size={36} />
          <View style={{ flex: 1 }}><Text style={styles.userName}>{u.name || 'Richfield member'}</Text><Text style={styles.userEmail}>{u.email}</Text></View>
          <View style={{ alignItems: 'flex-end' }}><Text style={styles.userRole}>{u.role === 'business' ? (u.businessDetails?.organizationName || 'Company') : u.role}</Text><View style={[styles.statusPill, u.verificationStatus === 'verified' ? styles.statusVerified : styles.statusPending]}><Text style={styles.statusText}>{u.verificationStatus}</Text></View></View>
        </View>
      ))}
    </View>
  );
  ...
  return (
    <View style={styles.container}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexDirection: 'row', marginTop: 8 }}>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {[
              ...
              { id: 'users', label: `Users (${realUsers.length})` },
              { id: 'moderation', label: `Moderation (${posts.filter((p) => !p.isShowcase).length})` },
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.card}>
              ...
              {pendingBusinesses.length === 0 ? <View style={styles.empty}><Text style={styles.emptyText}>All business accounts vetted and approved.</Text></View> : pendingBusinesses.map((biz) => (
                <View key={biz.id} style={styles.approvalRow}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap' }}><TouchableOpacity onPress={() => void runBusinessAiReview(biz)} style={styles.btnAi}><Sparkles color={theme.colors.navy} size={12} /><Text style={styles.btnAiText}>AI Review</Text></TouchableOpacity><TouchableOpacity onPress={() => handleRejectBusiness(biz.id)} style={styles.btnReject}><X color="#BE123C" size={12} /><Text style={styles.btnRejectText}>Reject</Text></TouchableOpacity><TouchableOpacity onPress={() => handleApproveBusiness(biz.id)} style={styles.btnApprove}><Check color="#fff" size={12} /><Text style={styles.btnApproveText}>Approve</Text></TouchableOpacity></View>
                </View>
              ))}
            </View>
            <View style={styles.card}>
        ...
        {activeTab === 'users' && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Richfield Identity Registry</Text><Text style={styles.cardSub}>Users are grouped by platform role for faster administration.</Text>
            {renderUserGroup('Students', students)}
            {renderUserGroup('Alumni', alumni)}
            {renderUserGroup('Companies', companies)}
            {renderUserGroup('Administrators', administrators)}
          </View>
        )}
          ...
          <View style={styles.card}>
            ...
            {posts.map((p) => (
              <View key={p.id} style={styles.moderationRow}>
                ...
                {p.isShowcase ? <View style={styles.sampleAdmin}><Text style={styles.sampleAdminText}>Sample</Text></View> : <TouchableOpacity onPress={() => handleDeletePost(p.id)} style={styles.btnReject}><Trash2 color="#BE123C" size={12} /><Text style={styles.btnRejectText}>Remove</Text></TouchableOpacity>}
              </View>
            ))}
          </View>
        )}
      ...
      <Modal visible={!!aiReviewTarget} transparent animationType="fade" onRequestClose={() => setAiReviewTarget(null)}>
        <View style={styles.modalOverlay}><View style={styles.aiModal}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}><View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}><Sparkles color={theme.colors.darkCyan} size={16} /><Text style={styles.cardTitle}>AI Verification Assistant</Text></View><TouchableOpacity onPress={() => setAiReviewTarget(null)}><Text style={styles.modalClose}>✕</Text></TouchableOpacity></View>
          <Text style={styles.cardSub}>{aiReviewTarget?.businessDetails?.organizationName || aiReviewTarget?.name}</Text>
          {aiLoading ? <View style={{ paddingVertical: 24, alignItems: 'center', gap: 8 }}><ActivityIndicator color={theme.colors.darkCyan} /><Text style={styles.cardSub}>Analysing submitted company evidence...</Text></View> : aiReview ? <>
            <View style={styles.aiScoreRow}><View><Text style={styles.aiScoreLabel}>Readiness score</Text><Text style={styles.aiScore}>{aiReview.score ?? 0}%</Text></View><View><Text style={styles.aiScoreLabel}>Review risk</Text><Text style={styles.aiRisk}>{aiReview.risk || 'Review'}</Text></View></View>
            <Text style={styles.approvalDesc}>{aiReview.summary}</Text>
            <View style={{ gap: 6 }}>{(aiReview.checks || []).map((check: any, index: number) => <View key={`${check.label}-${index}`} style={styles.aiCheck}><Text style={{ color: check.ok ? theme.colors.darkCyan : '#BE123C', fontWeight: '900' }}>{check.ok ? '✓' : '!'}</Text><Text style={styles.aiCheckText}>{check.label}</Text></View>)}</View>
            <View style={styles.aiDisclaimer}><Text style={styles.aiDisclaimerText}>{aiReview.disclaimer || 'AI-assisted review only. Final approval remains with the authorised administrator.'}</Text></View>
          </> : null}
        </View></View>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  sampleAdmin: { backgroundColor: '#FFF7ED', borderWidth: 1, borderColor: '#FED7AA', paddingHorizontal: 8, paddingVertical: 6, borderRadius: 8 },
  sampleAdminText: { color: '#9A3412', fontSize: 9, fontWeight: '800' },
  ...
  btnAi: { flexDirection: 'row', gap: 4, borderWidth: 1, borderColor: '#A5F3FC', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8, alignItems: 'center', backgroundColor: '#ECFEFF' },
  btnAiText: { fontSize: 10, fontWeight: '800', color: theme.colors.navy },
  ...
  userGroup: { borderTopWidth: 1, borderTopColor: theme.colors.slate100, paddingTop: 10, marginTop: 4, gap: 2 },
  userGroupHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
  userGroupTitle: { fontWeight: '900', fontSize: 11, color: theme.colors.navy },
  groupCount: { backgroundColor: theme.colors.slate100, borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2 },
  groupCountText: { fontSize: 9, fontWeight: '900', color: theme.colors.slate600 },
  groupEmpty: { fontSize: 10, color: theme.colors.slate400, paddingVertical: 8 },
  ...
  aiModal: { width: '100%', maxWidth: 560, backgroundColor: '#fff', borderRadius: 18, padding: 16, gap: 10, borderWidth: 1, borderColor: theme.colors.slate200 },
  modalClose: { fontSize: 18, color: theme.colors.slate500, fontWeight: '800' },
  aiScoreRow: { flexDirection: 'row', gap: 12 },
  aiScoreLabel: { fontSize: 9, color: theme.colors.slate500, fontWeight: '800', textTransform: 'uppercase' },
  aiScore: { fontSize: 22, fontWeight: '900', color: theme.colors.navy },
  aiRisk: { fontSize: 18, fontWeight: '900', color: theme.colors.darkCyan },
  aiCheck: { flexDirection: 'row', gap: 8, backgroundColor: theme.colors.slate50, padding: 8, borderRadius: 10, borderWidth: 1, borderColor: theme.colors.slate100 },
  aiCheckText: { flex: 1, fontSize: 10, color: theme.colors.slate700 },
  aiDisclaimer: { backgroundColor: '#FFFBEB', borderWidth: 1, borderColor: '#FDE68A', borderRadius: 10, padding: 8 },
  aiDisclaimerText: { fontSize: 9, lineHeight: 14, color: '#92400E', fontWeight: '700' },
});
