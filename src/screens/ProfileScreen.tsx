...
import { View, Text, StyleSheet, ScrollView, Image, TouchableOpacity, TextInput, Alert, Share } from 'react-native';
...
export default function ProfileScreen() {
  const { currentUser, handleUpdateProfile, handleAddEndorsement, setCurrentUser } = useApp();
  ...
  const saveProfile = () => { handleUpdateProfile({ headline, summary }); setIsEditing(false); };
  ...
  const handleExport = async () => {
    const content = `RICHFIELD COLLEGE - VERIFIED STUDENT DOSSIER
Name: ${user.name}
      ...
      <View style={styles.headerCard}>
        ...
        {isEditing ? (
          <View style={styles.editBox}>
            <Text style={styles.label}>Headline</Text><TextInput value={headline} onChangeText={setHeadline} style={styles.input} placeholderTextColor={theme.colors.slate400} />
            <Text style={styles.label}>About / Summary</Text><TextInput value={summary} onChangeText={setSummary} style={[styles.input, { height: 80, textAlignVertical: 'top', paddingTop: 10 }]} multiline placeholderTextColor={theme.colors.slate400} />
            <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'flex-end' }}><TouchableOpacity onPress={() => setIsEditing(false)} style={styles.btnGhost}><Text style={styles.btnGhostText}>Cancel</Text></TouchableOpacity><TouchableOpacity onPress={saveProfile} style={styles.btnPrimary}><Text style={styles.btnPrimaryText}>Save Changes</Text></TouchableOpacity></View>
          </View>
        ) : (
          <View style={{ gap: 6 }}>
          ...
          <View style={styles.card}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
              {[
                ...
                { label: 'Candidate Pool', value: '1,842 Students' },
                { label: 'Applications', value: '124 Total' },
                { label: 'Shortlist Rate', value: '38% Qualified' },
              ].map((s) => <View key={s.label} style={styles.stat}><Text style={styles.statLabel}>{s.label}</Text><Text style={styles.statValue}>{s.value}</Text></View>)}
            </View>
          </View>
        </>
      ) : (
